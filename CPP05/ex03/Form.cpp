...
Form::Form() : _name("unnamed_form"), _signed(false), _required_sign_grade(150), _required_exec_grade(150), _target("")
{}

Form::Form(const std::string &name, const int required_sign_grade, const int required_exec_grade)
    throw(GradeTooHighException, GradeTooLowException)
    : _name(name), _signed(false), _required_sign_grade(required_sign_grade), _required_exec_grade(required_exec_grade), _target("")
{
...
: _name(other.getName()), _signed(other.getSigned()), _required_sign_grade(other.getRequiredSignGrade()),  _required_exec_grade(other.getRequiredExecGrade()), _target(other.getTarget())
{ }
...
const throw(GradeTooLowException)
{
    if (executor.getGrade() > this->_required_exec_grade)
        throw GradeTooLowException();
    if (this->_signed)
        this->_on_execute();
}

void                Form::_on_execute() const
{ }
